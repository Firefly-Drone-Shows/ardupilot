...
NOINLINE void Copter::send_heartbeat(mavlink_channel_t chan)
{
    ...
    ahrs.get_position(curr_amsl_loc);

/*    gcs_chan[chan-MAVLINK_COMM_0].send_heartbeat_as_data16(get_frame_mav_type(),
                                            base_mode,
                                            custom_mode,
                                            system_status,
                                            current_loc.lat,               
                                            current_loc.lng,     
                                            static_cast<int16_t>(curr_amsl_loc.alt / 10), 
                                            static_cast<uint16_t>((ahrs.yaw_sensor / 100) % 360),
                                            gps.time_epoch_usec()); */
...
void NOINLINE Copter::send_location(mavlink_channel_t chan)
{
    ...
    mavlink_msg_global_position_int_send(
        ...
        (curr_amsl_loc.alt) * 10UL,      // millimeters above sea level
...
void NOINLINE Copter::send_dance_status(mavlink_channel_t chan)
{   
    Location true_gps;
    ahrs.get_position(true_gps);

    Vector3f target;
    dance_nav->get_target_position(0.01, target);
    ...
    mavlink_msg_hil_state_send(
        ...
        target.y,               // as pitch
        target.z,               // as yaw
        dance_nav->get_max_altitude(),  // as rollspeed
        dance_nav->get_max_distance_from_center(), // as pitchspeed
        dance_nav->get_fps(),                      // as yawspeed
        dance_nav->get_dance_starting_time(),      // as lat
        dance_nav->get_time_to_start() / 1000, // seconds   // as lon
        true_gps.alt,                               // as alt
	    dance_nav->get_max_horizontal_speed(), //cm per sec,   // as vx
        dance_nav->get_path_length(),               // as vy
        dance_nav->ready_to_dance(),                // as vz
        dance_nav->get_max_vertical_speed(),   //cm per sec   // as xacc
        dance_nav->get_takeoff_offset(),          // yacc
        dance_nav->get_moving_to_position_offset());        // as zacc

}


void NOINLINE Copter::send_dance_msg_92(mavlink_channel_t chan){

    uint8_t payload[16] = {0};
    // bytes #0-1: yaw in degrees (0 - 360)
    int32_t heading = static_cast<int32_t>(ahrs.yaw_sensor / 100) % 360;
    if (heading < 0) { heading = heading + 360; }
    uint16_t heading_positive = static_cast<uint16_t>(heading);
    payload[0] = (heading_positive >> 8) & 0xFF;
    payload[1] = heading_positive & 0xFF;

    // bytes #2-5: lat
    payload[2] = (current_loc.lat >> 24) & 0xFF;
    payload[3] = (current_loc.lat >> 16) & 0xFF;
    payload[4] = (current_loc.lat >> 8) & 0xFF;
    payload[5] = current_loc.lat & 0xFF;

    // bytes #6-9: lon
    payload[6] = (current_loc.lng >> 24) & 0xFF;
    payload[7] = (current_loc.lng >> 16) & 0xFF;
    payload[8] = (current_loc.lng >> 8) & 0xFF;
    payload[9] = current_loc.lng & 0xFF;

    // bytes #10-11: ahrs alt in decimeters
    Location ahrs_loc;
    ahrs.get_position(ahrs_loc);
    int16_t alt = static_cast<int16_t>(ahrs_loc.alt / 10);
    payload[10] = (alt >> 8) & 0xFF;
    payload[11] = alt & 0xFF;

    // bytes #12-13: raw gps alt in decimeters
    const Location &gps_loc = gps.location(0);
    int16_t gps_alt = static_cast<int16_t>(gps_loc.alt / 10);
    payload[12] = (gps_alt >> 8) & 0xFF;
    payload[13] = gps_alt & 0xFF;

    // bytes #14-15: battery voltage
    uint16_t vlt = static_cast<uint16_t>(battery.voltage() * 1000);
    payload[14] = (vlt >> 8) & 0xFF;
    payload[15] = vlt & 0xFF;


    mavlink_msg_data16_send(
        chan,
        92,              // Telemetry packet id
        16,     // payload length
        payload);    
...
void NOINLINE Copter::send_dance_msg_93(mavlink_channel_t chan){

    uint8_t payload[16] = {0};
    // bytes #0-2: time of day in seconds
    uint64_t time_usec = gps.time_epoch_usec();
    uint64_t time_of_day_in_seconds = (time_usec / 1000000) % 86400;
    payload[0] = (time_of_day_in_seconds >> 16) & 0xFF;
    payload[1] = (time_of_day_in_seconds >> 8) & 0xFF;
    payload[2] = time_of_day_in_seconds & 0xFF;

    // byte 3: num sats
    payload[3] = gps.num_sats();

    // byte 4: gps fix
    payload[4] = gps.status();

    // byte 5: dance state: 
    payload[5] = get_dance_state();

    // byte 6-9 dance time to start
    uint32_t sec_to_start = (dance_nav->get_time_to_start() / 1000);
    payload[6] = (sec_to_start >> 24) & 0xFF;
    payload[7] = (sec_to_start >> 16) & 0xFF;
    payload[8] = (sec_to_start >> 8) & 0xFF;
    payload[9] = sec_to_start & 0xFF;

    // byte 10-13: dance time to land
    int32_t sec_to_land = dance_nav->get_seconds_to_land();
    payload[10] = (sec_to_land >> 24) & 0xFF;
    payload[11] = (sec_to_land >> 16) & 0xFF;
    payload[12] = (sec_to_land >> 8) & 0xFF;
    payload[13] = sec_to_land & 0xFF;

    // byte 14-15 dance ready flag
    int16_t dance_ready_flag = dance_nav->ready_to_dance();
    payload[14] = (dance_ready_flag >> 8) & 0xFF;
    payload[15] = dance_ready_flag & 0xFF;

    // rtcm recevie status
    uint8_t rtcm_status = copter.rtcm_counter.get_state(0);

    //hal.console->printf("Fist Status :%d\n", rtcm_status );

    mavlink_msg_data16_send(
        chan,
        93,              // Telemetry packet id
        rtcm_status,      // RTCM channel status here
        payload);    

}
  

void NOINLINE Copter::send_dance_msg_94(mavlink_channel_t chan){
    ...
    // byte 0-11 target pos, x-y-z
    // ATTENTION: Do we really need it??
    Vector3f target;
    dance_nav->get_target_position(0.01, target);
    float_payload.f = target.x;
    payload[0] = float_payload.buf[0];
    payload[1] = float_payload.buf[1];
    payload[2] = float_payload.buf[2];
    payload[3] = float_payload.buf[3];
    float_payload.f = target.y;
    payload[4] = float_payload.buf[0];
    payload[5] = float_payload.buf[1];
    payload[6] = float_payload.buf[2];
    payload[7] = float_payload.buf[3];
    float_payload.f = target.z;
    payload[8] = float_payload.buf[0];
    payload[9] = float_payload.buf[1];
    payload[10] = float_payload.buf[2];
    payload[11] = float_payload.buf[3];
    
    // dance max altitude
    float_payload.f = dance_nav->get_max_altitude();
    payload[12] = float_payload.buf[0];
    payload[13] = float_payload.buf[1];
    payload[14] = float_payload.buf[2];
    payload[15] = float_payload.buf[3];

    // dance min altitude
    int16_t payload_min_alt = static_cast<int16_t>(dance_nav->get_min_altitude() / 10); 
    payload[16] = (payload_min_alt >> 8) & 0xFF; 
    payload[17] = payload_min_alt & 0xFF; 

    // dance max distance from center
    float_payload.f = dance_nav->get_max_distance_from_center();
    payload[18] = float_payload.buf[0];
    payload[19] = float_payload.buf[1];
    payload[20] = float_payload.buf[2];
    payload[21] = float_payload.buf[3];

    // dance max hor speed
    int16_t payload_max_hor_speed = dance_nav->get_max_horizontal_speed();
    payload[22] = (payload_max_hor_speed >> 8) & 0xFF;
    payload[23] = payload_max_hor_speed & 0xFF;

    // dance max ver speed
    int16_t payload_max_ver_speed = dance_nav->get_max_vertical_speed();
    payload[24] = (payload_max_ver_speed >> 8) & 0xFF;
    payload[25] = payload_max_ver_speed & 0xFF;

    // dance max ang speed
    uint16_t payload_max_ang_speed = dance_nav->get_max_angular_speed();
    payload[26] = (payload_max_ang_speed >> 8) & 0xFF;
    payload[27] = payload_max_ang_speed & 0xFF;

    // dance fps
    float_payload.f = dance_nav->get_fps();
    payload[28] = float_payload.buf[0];
    payload[29] = float_payload.buf[1];
    payload[30] = float_payload.buf[2];
    payload[31] = float_payload.buf[3];
    
    // path length
    int16_t payload_path_length = dance_nav->get_path_length(); 
    payload[32] = (payload_path_length >> 8) & 0xFF;
    payload[33] = payload_path_length & 0xFF;

    // finish mode, landing type. 0 - land, 1 - rtl.
    payload[34] = static_cast<uint8_t>(dance_nav->return_to_home_position());

    // dance type
    payload[35] = dance_nav->get_type();

    // fence crc
    uint16_t fence_crc = fence.calc_fence_crc();
    payload[36] = (fence_crc >> 8) & 0xFF;
    payload[37] = fence_crc & 0xFF;

    // dance starting time
    int32_t payload_dance_starting_time = dance_nav->get_dance_starting_time();
    payload[38] = (payload_dance_starting_time >> 16) & 0xFF;
    payload[39] = (payload_dance_starting_time >> 8) & 0xFF;
    payload[40] = payload_dance_starting_time & 0xFF;

    // dance takeoff offset time
    int16_t payload_takeoff_offset = dance_nav->get_takeoff_offset();
    payload[41] = (payload_takeoff_offset >> 8) & 0xFF;
    payload[42] = payload_takeoff_offset & 0xFF;

    // dance moving to position offset time
    int16_t payload_moving_to_position_offset = dance_nav->get_moving_to_position_offset();
    payload[43] = (payload_moving_to_position_offset >> 8) & 0xFF;
    payload[44] = payload_moving_to_position_offset & 0xFF;
    
    // dance landing offset time
    int16_t payload_land_offset = dance_nav->get_land_offset();
    payload[45] = (payload_land_offset >> 8) & 0xFF;
    payload[46] = payload_land_offset & 0xFF;

    // relative takeoff alt (int32_t, cm)
    int32_t payload_takeoff_alt = static_cast<int32_t>(dance_nav->get_takeoff_altitude());
    payload[47] = (payload_takeoff_alt >> 24) & 0xFF;
    payload[48] = (payload_takeoff_alt >> 16) & 0xFF;
    payload[49] = (payload_takeoff_alt >> 8) & 0xFF;
    payload[50] = payload_takeoff_alt & 0xFF;

    // target (first point) coord (ATTENTION: ambigious with target POS)
    Location pos = dance_nav->get_first_point_position();
    payload[51] = (pos.lat >> 24) & 0xFF;
    payload[52] = (pos.lat >> 16) & 0xFF;
    payload[53] = (pos.lat >> 8) & 0xFF;
    payload[54] = pos.lat & 0xFF;
    payload[55] = (pos.lng >> 24) & 0xFF;
    payload[56] = (pos.lng >> 16) & 0xFF;
    payload[57] = (pos.lng >> 8) & 0xFF;
    payload[58] = pos.lng & 0xFF;
    int32_t payload_target_alt = static_cast<int32_t>(pos.alt);
    payload[59] = (payload_target_alt >> 24) & 0xFF;
    payload[60] = (payload_target_alt >> 16) & 0xFF;
    payload[61] = (payload_target_alt >> 8) & 0xFF;
    payload[62] = payload_target_alt & 0xFF;
   

    mavlink_msg_data64_send(
        chan,
        94,              // Telemetry packet id
        64,              // payload length
        payload);    

}

void NOINLINE Copter::send_dance_msg_95(mavlink_channel_t chan){

    uint8_t payload[16] = {0};

    // vibrations
    Vector3f vibration = ins.get_vibration_levels();
    int16_t vib = static_cast<int16_t>(vibration.x * 10);
    payload[0] = (vib >> 8) & 0xFF;
    payload[1] = vib & 0xFF;
    vib = static_cast<int16_t>(vibration.y * 10);
    payload[2] = (vib >> 8) & 0xFF;
    payload[3] = vib & 0xFF;
    vib = static_cast<int16_t>(vibration.z * 10);
    payload[4] = (vib >> 8) & 0xFF;
    payload[5] = vib & 0xFF;

    // speed
    const Vector3f &vel = inertial_nav.get_velocity();
    uint16_t hor_vel = static_cast<uint16_t>(sqrt(vel.x * vel.x + vel.y * vel.y));
    payload[6] = (hor_vel >> 8) & 0xFF;
    payload[7] = hor_vel & 0xFF;
    int16_t ver_vel = vel.z;
    payload[8] = (ver_vel >> 8) & 0xFF;
    payload[9] = ver_vel & 0xFF;

    // time since boot
    uint32_t time_boot = AP_HAL::millis();
    payload[10] = (time_boot >> 24) & 0xFF;
    payload[11] = (time_boot >> 16) & 0xFF;
    payload[12] = (time_boot >> 8) & 0xFF;
    payload[13] = time_boot & 0xFF;

    // rtcm recevie status
    uint8_t rtcm_status = copter.rtcm_counter.get_state(1);

    //hal.console->printf("Second Status :%d\n", rtcm_status);

    mavlink_msg_data16_send(
        chan,
        95,              // Telemetry packet id
        rtcm_status,     // RTCM state for alternative
        payload);    

}

void NOINLINE Copter::send_dance_additional_status(mavlink_channel_t chan)
{   
    uint8_t payload[32] = {0};

    // byte #0: landing type. 0 - land, 1 - rtl.
    payload[0] = static_cast<uint8_t>(dance_nav->return_to_home_position());

    // bytes #1-4: relative takeoff alt (int32_t, cm)
    int32_t alt = static_cast<int32_t>(dance_nav->get_takeoff_altitude());
    payload[1] = (alt >> 24) & 0xFF;
    payload[2] = (alt >> 16) & 0xFF;
    payload[3] = (alt >> 8) & 0xFF;
    payload[4] = alt & 0xFF;
    ...
    // byte #6-7 
    int16_t land_offset = dance_nav->get_land_offset();
    payload[6] = (land_offset >> 8) & 0xFF;
    payload[7] = land_offset & 0xFF;

    // byte 8-9: min relative alt during path (in decimeters) 
    int16_t min_alt = static_cast<int16_t>(dance_nav->get_min_altitude() / 10); 
    payload[8] = (min_alt >> 8) & 0xFF; 
    payload[9] = min_alt & 0xFF; 
    ...
    uint16_t fence_crc = fence.calc_fence_crc();
    payload[14] = (fence_crc >> 8) & 0xFF;
    payload[15] =  fence_crc & 0xFF;

    int16_t ang = dance_nav->get_max_angular_speed();
    payload[16] = (ang >> 8) & 0xFF;
    payload[17] = ang & 0xFF;

    payload[18] = dance_nav->get_type();

    mavlink_msg_data32_send(
        chan,
        51,              // Telemetry packet id
        32,              // payload length
        payload);        
...
void NOINLINE Copter::send_dance_target_position(mavlink_channel_t chan)
{
    Location pos = dance_nav->get_first_point_position();
   
    float alt = pos.alt / 100;
    // add centimeters
    alt = alt + (float)(pos.alt % 100) / 100.0;
...
bool GCS_MAVLINK_Copter::try_send_message(enum ap_message id)
{
    ...
    case MSG_BATTERY_STATUS:
        ...
        break;
    case MSG_HIL_STATE:
        copter.send_dance_status(chan);
        break;
    case MSG_DANCE1:
        copter.send_dance_additional_status(chan);
        break;   
    case MSG_DANCE_TOP:
        copter.dance_main_telemetry_count[chan]++;
        if (copter.dance_nav->get_telemetry_main_rate() > 0) {
            if (copter.dance_main_telemetry_count[chan] % (copter.dance_nav->get_telemetry_main_rate() * 2) == 0) {
                copter.send_dance_msg_92(chan);
            }
        }
        break;
    case MSG_DANCE_GPS:
        if (copter.dance_nav->get_telemetry_main_rate() > 0) {
            if ((copter.dance_main_telemetry_count[chan] + copter.dance_nav->get_telemetry_main_rate()) % (copter.dance_nav->get_telemetry_main_rate() * 4) == 0) {
                copter.send_dance_msg_93(chan);
            }
        }
        break;
     case MSG_DANCE_SPEED:
        if (copter.dance_nav->get_telemetry_main_rate() > 0) {
            if ((copter.dance_main_telemetry_count[chan] + copter.dance_nav->get_telemetry_main_rate() * 3) % (copter.dance_nav->get_telemetry_main_rate() * 4) == 0) {
                copter.send_dance_msg_95(chan);
            }
        }
        break;     
    case MSG_DANCE_PREFLIGHT:
        copter.dance_preflight_telemetry_count[chan]++;    
        if (copter.dance_nav->get_telemetry_preflight_rate() > 0) {
            if (copter.dance_preflight_telemetry_count[chan] % copter.dance_nav->get_telemetry_preflight_rate() == 0) {
                copter.send_dance_msg_94(chan);
            }
        }
        break;        
      
    case MSG_POSITION_TARGET_GLOBAL_INT:
        copter.send_dance_target_position(chan);
        break;
    }
...
const AP_Param::GroupInfo GCS_MAVLINK::var_info[] = {
    ...
    // @Description: Stream rate of GLOBAL_POSITION_INT and LOCAL_POSITION_NED to ground station. Also stream SYSTEM_TIME
    // @Units: Hz
    // @Range: 0 10
    // @Increment: 1
    // @User: Advanced
    ...
    // @Description: Stream rate of AHRS, HWSTATUS, RANGEFINDER, DISTANCE_SENSOR, TERRAIN_REQUEST, BATTERY2, MOUNT_STATUS, OPTICAL_FLOW, GIMBAL_REPORT, MAG_CAL_REPORT, MAG_CAL_PROGRESS, EKF_STATUS_REPORT, VIBRATION and RPM to ground station
    // @Units: Hz
    // @Range: 0 10
    // @Increment: 1
    // @User: Advanced
    ...
    AP_GROUPINFO("FRWD_ENABLE",   10, GCS_MAVLINK, enable_forwarding,  0),
...
GCS_MAVLINK_Copter::data_stream_send(void)
{
    ...
    // FIRST THINGS FIRST. EXTRA2 - Dance compact telemetry
    if (stream_trigger(STREAM_EXTRA2)) {
        //send_message(MSG_VFR_HUD);
        send_message(MSG_DANCE_TOP);
        send_message(MSG_DANCE_GPS);
        send_message(MSG_DANCE_PREFLIGHT);
        send_message(MSG_DANCE_SPEED);
    ...
    // if dance telemetry mode is turned on
    // skip all other telemetry
    if (copter.dance_nav->get_telemetry_mode() > 0 && chan > 0) {
        // check if STREAM_EXTRA2 is turned on (it should be, 
        // when we are in dance telemetry mode)
        if ((uint8_t)streamRates[STREAM_EXTRA2].get() == 0) {
            streamRates[STREAM_EXTRA2].set(1);
        }
        return;
    }
    ...
    if (stream_trigger(STREAM_POSITION)) {
        ...
        send_message(MSG_GPS_RAW);
        send_message(MSG_HIL_STATE);
        send_message(MSG_POSITION_TARGET_GLOBAL_INT);
        send_message(MSG_VIBRATION);
        send_message(MSG_DANCE1);
    }
...
void GCS_MAVLINK_Copter::handleMessage(mavlink_message_t* msg)
{
    ...
    case MAVLINK_MSG_ID_PARAM_REQUEST_LIST:         // MAV ID: 21
    {
        ...
        if (copter.dance_nav->get_path_success()) {
            GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_INFO, "PATH #%d, LOADED %d POINTS", copter.dance_nav->get_path_id(), copter.dance_nav->get_path_length());
        } else {
            GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_INFO, "PATH #%d, FAILED WITH %d POINTS", copter.dance_nav->get_path_id(), copter.dance_nav->get_path_length());
        }
    ...
    case MAVLINK_MSG_ID_COMMAND_LONG:       // MAV ID: 76
    {
        ...
        case MAV_CMD_PREFLIGHT_REBOOT_SHUTDOWN:
            ...
            if (copter.motors->armed()) {
                GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_CRITICAL, "Can not reboot. Vehicle is armed");
                result = MAV_RESULT_FAILED;
                break;
            }
            ...
            if (!copter.ap.land_complete) {
                GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_CRITICAL, "Can not reboot. Vehicle is airborne");
                result = MAV_RESULT_FAILED;
                break;
            }
        ...
        case MAV_CMD_USER_1: {
            result = MAV_RESULT_ACCEPTED;
            uint8_t r = static_cast<uint8_t>(packet.param1);
            if (packet.param1 < 0) {
                r = 0;
            }
            uint8_t g = static_cast<uint8_t>(packet.param2);
            if (packet.param2 < 0) {
                g = 0;
            }
            uint8_t b = static_cast<uint8_t>(packet.param3);
            if (packet.param3 < 0) {
                b = 0;
            }

            uint16_t dura = static_cast<uint16_t>(packet.param4);
            if (packet.param4 < 0) {
                dura = 0;
            }
            uint8_t rat = static_cast<uint8_t>(packet.param5);
            if (packet.param5 < 0) {
                rat = 0;
            }
             uint8_t group = static_cast<uint8_t>(packet.param6);
            if (packet.param6 < 0) {
                group = 0;
            }
            
            copter.dance_nav->handle_set_color(r, g, b, dura, rat, group);
            break;
        }
    ...
    {
      result = MAV_RESULT_ACCEPTED;
      copter.gps.handle_msg(chan, msg);
      break;
    }
    ...
    case MAVLINK_MSG_ID_GPS_INJECT_DATA:
        handle_gps_inject(chan, msg, copter.gps);
        result = MAV_RESULT_ACCEPTED;
        break;
    ...
    case MAVLINK_MSG_ID_DATA32:
        ...
        if (msg != nullptr) {
            mavlink_data32_t packet_data32;
            mavlink_msg_data32_decode(msg, &packet_data32);    
            if (packet_data32.type == 61) {
                if (copter.fence.handle_msg_data32(chan, packet_data32)) { 
                    result = MAV_RESULT_ACCEPTED;
                }
            }
        }
        mavlink_msg_command_ack_send_buf(msg, chan, MAVLINK_MSG_ID_DATA32, result);
        break;
    ...
    case MAVLINK_MSG_ID_V2_EXTENSION:
        copter.dance_nav->handle_data_v2(chan, msg);
        break;
    
    case MAVLINK_MSG_ID_DATA96:
        copter.gps.handle_msg(chan, msg);
        copter.dance_nav->handle_data_v3(chan, msg);
        break;
