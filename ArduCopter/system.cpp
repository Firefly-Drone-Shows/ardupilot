...
void Copter::init_ardupilot()
{
    ...
    // Change AUX1-6 PWM Freq to AUX_FREQ Hz
    uint32_t chmask = 0;
    for (int i = 8; i < 14; ++i ) {
            hal.rcout->enable_ch(i);
            chmask |= 1 << i;
    }
    hal.rcout->set_freq(chmask, g.aux_freq);
...
void Copter::allocate_motors(void)
{
    ...
    dancing_led = new AC_Dancing_LED();
    if (dancing_led == nullptr) {
        AP_HAL::panic("Unable to allocate Dancing LEDs");
    }
    AP_Param::load_object_from_eeprom(dancing_led, dancing_led->var_info);

    dance_nav = new AC_Dance(inertial_nav, *ahrs_view, *pos_control, fence, *dancing_led);
    if (dance_nav == nullptr) {
        AP_HAL::panic("Unable to allocate DanceNav");
    }

    AP_Param::load_object_from_eeprom(dance_nav, dance_nav->var_info);
