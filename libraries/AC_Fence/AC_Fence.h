...
#define AC_FENCE_MANUAL_RECOVERY_TIME_MIN           500     // pilot has 0.5 seconds to recover during which time the autopilot will not attempt to re-take control

#define AC_FENCE_HARD_DIST_DEFAULT                  30       // defaul hard fence offset
#define AC_FENCE_HARD_BREACH_TIMEOUT                1000     // delay before engines will be shutdown after hard fence breach
...
public:
    ...
    bool hard_fence_enabled() const {return _hard_fence_enable;}
    ...
    /// handler for polygon fence messages (DATA32) with GCS
    bool handle_msg_data32(mavlink_channel_t chan, const mavlink_data32_t &packet);

    static const struct AP_Param::GroupInfo var_info[];


    /// checks if hard fence has been broken to initialize disarming process even inflight
    bool hard_fence_breach();

    float distance_to_poly_min(Vector2f *pos, uint8_t boundary_num, Vector2f *boundary);

    uint16_t calc_fence_crc();
private:
    ...
    AP_Float        _hard_dist;             // the distance outside the fence, when motors will be disarmed.
    AP_Int8         _hard_fence_enable;     // if true, Hard Fence is enabled.

    bool            _hard_fence_breach;     // true if Hard fence breach
    uint32_t        _hard_fence_breach_time;
    ...
    float           _poly_breach_distance;      // distance beyond polygonal fence
