...
void AP_GPS::handle_msg(mavlink_channel_t chan, const mavlink_message_t *msg)
{
    if (msg->msgid == MAVLINK_MSG_ID_GPS_RTCM_DATA) {
        ...
        return;
    } else if (msg->msgid == MAVLINK_MSG_ID_DATA96) {
        handle_data96(chan, msg);
        return;
    }
...
void AP_GPS::handle_gps_rtcm_data(mavlink_channel_t chan, const mavlink_message_t *msg)
{
    ...
    if ((packet.flags & 1) == 0) {
        ...
        log_rtcm(chan, msg->msgid, packet.data, packet.len);
        return;
    }
    ...
    */
    ...
    // We think it's ok to rewrite RTCM fragmet if it already have been received.
    // Old realization can have problems when works with two channels
    // simultaneously
    if (rtcm_buffer->fragments_received && 
            rtcm_buffer->sequence != sequence) { 
    ...
    if (rtcm_buffer->fragment_count != 0 &&
        ...
        log_rtcm(chan, msg->msgid, rtcm_buffer->buffer, rtcm_buffer->total_length);
        memset(rtcm_buffer, 0, sizeof(*rtcm_buffer));
    }
}

void AP_GPS::handle_data96(mavlink_channel_t chan, const mavlink_message_t *msg)
{
    ...
    if (packet.len > DATA96_MSGLEN) {
        // invalid packet size
        return;
    }

    // see if we need to allocate re-assembly buffer
    if (data96_buffer == nullptr) {
        data96_buffer = (struct data96_buffer *)calloc(1, sizeof(*data96_buffer));
        if (data96_buffer == nullptr) {
            // nothing to do but discard the data
            return;
        }
    ...
    // rtcm sequence number. All chunks of single RTCM message
    // must have equal sequence value
    uint8_t sequence = getbitu(packet.data, 24, 8);
    
    // fragments are counting from 0. Maximum fragment value = 7
    uint16_t fragment = getbitu(packet.data, 32, 8);

    // max value = 8 (DATA96_MAX_CHUNKS)
    uint8_t nopackets = getbitu(packet.data, 40, 8);

    // sanity check for nopackets and current fragment number
    if (nopackets > DATA96_MAX_FRAGMENTS || nopackets == 0 || fragment > DATA96_MAX_FRAGMENTS - 1) {
            return;
    }

    // see if this fragment is consistent with existing fragments
    /* 
    if (data96_buffer->fragments_received &&
        (data96_buffer->sequence != sequence ||
         data96_buffer->fragment_count != nopackets ||
        (data96_buffer->fragments_received & (1U<<fragment)))) { 
    */
    ...
    if (data96_buffer->fragments_received &&
        ...
        // we have one or more partial fragments already received
        // which conflict with the new fragment, discard previous fragments
        memset(data96_buffer, 0, sizeof(*data96_buffer));
    }

    // update sequence
    data96_buffer->sequence = sequence;

    // unlike rtcm handling we know the number of packets directly from data96 message
    data96_buffer->fragment_count = nopackets;

    // add this fragment. Set corresponding bit to fragments_received.
    // Maximum fragment value = 7, so (1U << fragment) should be ok
    data96_buffer->fragments_received |= (1U << fragment);

    // copy the data
    memcpy(&data96_buffer->buffer[DATA96_MSGLEN * (uint16_t)fragment], &packet.data[6], packet.len);

    // when we get a fragment is last we can calculate exact length of RTCM message
    if (fragment == data96_buffer->fragment_count - 1) {
        data96_buffer->total_length = (DATA96_MSGLEN * fragment) + packet.len;
    }

    // see if we have all fragments
    if (data96_buffer->fragment_count != 0 &&
        data96_buffer->fragments_received == (1U << data96_buffer->fragment_count) - 1) {
        // we have them all, inject

        inject_data(data96_buffer->buffer, data96_buffer->total_length);
        log_rtcm(chan, msg->msgid, data96_buffer->buffer, data96_buffer->total_length);
        memset(data96_buffer, 0, sizeof(*data96_buffer));
    }
}
...
void AP_GPS::log_rtcm(mavlink_channel_t chan, uint16_t msg_id, uint8_t *buffer, uint16_t length) {
    int8_t tmp_index = chan - MAVLINK_COMM_0;

    // Check if we are in limit  [0 - 8)
    if (tmp_index < 0 || tmp_index >= MAX_CHANNELS_NUM) {
        return;
    }

    rtcm_package_counter[tmp_index]++;

    rtcm_channel_mask |= 1 << (tmp_index);

    if (_DataFlash == nullptr) {
        return;
    }

    if (length <= 6) // Minimal rtcm pachage length in bytes
        return;

    uint8_t i = 24;
    uint16_t type = getbitu(buffer, i, 12);
    i += 12;
    uint16_t stationId = getbitu(buffer, i, 12);
    i += 12;

    uint64_t now = AP_HAL::micros64();
    struct log_RTCM log_msg = {
    ...
    _DataFlash->WriteBlock(&log_msg, sizeof(log_msg));
}

uint16_t AP_GPS::get_rtcm_count(uint8_t chan) const {
    if (chan >= MAX_CHANNELS_NUM)
        return 0;
    else
        return rtcm_package_counter[chan];
}

uint8_t AP_GPS::get_rtcm_channel_mask() const {
    return rtcm_channel_mask;
}
