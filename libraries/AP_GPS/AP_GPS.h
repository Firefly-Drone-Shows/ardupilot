...
#define DATA96_MAX_FRAGMENTS 8
#define DATA96_MSGLEN 90
...
public:
   ...
   enum GPS_Config {
    ...
    void log_rtcm(mavlink_channel_t chan, uint16_t msg_id, uint8_t *buffer, uint16_t length);

    uint16_t get_rtcm_count(uint8_t chan) const;

    uint8_t get_rtcm_channel_mask() const;
...
private:
    ...
    uint16_t rtcm_package_counter[MAX_CHANNELS_NUM];
    uint8_t rtcm_channel_mask{0};

      // re-assemble GPS_RTCM_DATA message
    /*
      buffer for re-assembling RTCM data for GPS injection, coming
      in DATA96 message.

      The rtcm_buffer is allocated on first use. Once a block of data
      is successfully reassembled it is injected into all active GPS
      backends. This assumes we don't want more than 8*90=720 bytes
      in a RTCM data block
     */
    struct data96_buffer {
        uint8_t fragments_received;
        uint8_t sequence;
        uint8_t fragment_count;
        uint16_t total_length;
        uint8_t buffer[DATA96_MSGLEN * DATA96_MAX_FRAGMENTS];
    } *data96_buffer;


    // re-assemble GPS_RTCM_DATA message
    void handle_gps_rtcm_data(mavlink_channel_t chan, const mavlink_message_t *msg);
    // re-assemble DATA96 message
    void handle_data96(mavlink_channel_t chan, const mavlink_message_t *msg);
