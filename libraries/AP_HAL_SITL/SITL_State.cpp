...
void SITL_State::_sitl_setup(const char *home_str)
{
    ...
    if (_sitl != nullptr) {
        ...
        asprintf(&filename, "/home/mike/FLIGHTS/%d.FLIGHT", _instance);
        flight_file = ::fopen(filename, "w");
        _f_last_ts = 0; */
        }
...
void SITL_State::_output_to_flightgear(void)
{
 //   SITL::FGNetFDM fdm {};
    ...
    /*fdm.version = 0x18;
    fdm.padding = 0;
    fdm.longitude = sfdm.longitude;
    fdm.latitude = sfdm.latitude;
    ...
    // 25 FPS
    /*if (_f_last_ts + 40000 < sfdm.timestamp_us) {
        _fdata.altitude = sfdm.altitude * 100;
        _fdata.latitude = sfdm.latitude * 10000000;
        _fdata.longitude = sfdm.longitude * 10000000;
        _fdata.pitch = sfdm.pitchDeg * 100;
        _fdata.roll = sfdm.rollDeg * 100;
        _fdata.yaw = sfdm.yawDeg * 100;
        _fdata.ts = hal.util->get_system_clock_ms();
        _fdata.r = pwm_output[5];
        _fdata.g = pwm_output[6];
        _fdata.b = pwm_output[7];
        _f_last_ts = sfdm.timestamp_us;
        ::fwrite(&_fdata, sizeof(char), sizeof(_fdata), flight_file); 
    } */
}
