...
void 
GCS_MAVLINK::handle_gps_inject(mavlink_channel_t rtcm_chan, const mavlink_message_t *msg, AP_GPS &gps)
{
    mavlink_gps_inject_data_t packet;
    mavlink_msg_gps_inject_data_decode(msg, &packet);
    //TODO: check target

    gps.inject_data(rtcm_chan, packet.data, packet.len);
                                 ...
                                 mavlink_message_t &msg)
{
  ...
  //  bool checked_and_forwarded = false;

    //if (enable_forwarding.get() > 0) {
        //checked_and_forwarded  = routing.check_and_forward(chan, &msg);
    //} else {
      //  checked_and_forwarded  = true;
    //}

    if (routing.check_and_forward(chan, &msg, enable_forwarding.get()) &&
...
void GCS::send_statustext(MAV_SEVERITY severity, uint8_t dest_bitmask, const char *text)
{
    ...
    AP_Param *vp_mute;
    enum ap_var_type var_type_mute;
    char key_mute[AP_MAX_NAME_SIZE+1];
    strncpy(key_mute, "DANCE_MUTE", AP_MAX_NAME_SIZE);
    key_mute[AP_MAX_NAME_SIZE] = 0;
    // find DANCE_MUTE param. If it exists and > 0 - mute the messages
    vp_mute = AP_Param::find(key_mute, &var_type_mute);
    if (vp_mute != nullptr) {
        float val = vp_mute->cast_to_float(var_type_mute);
        if (val > 0) {
            hal.console->printf("MUTED: %s\n", text);
            return;
        }
    } else {
        hal.console->printf("No PARAM!\n");
    }
...
void GCS_MAVLINK::send_autopilot_version(uint8_t major_version, uint8_t minor_version, uint8_t patch_version, uint8_t version_type) const
{
    ...
    // DDC Spec version 3.3.1.1
    os_sw_version = 3 << 24 | 3 << 16 | 1 << 8 | 1 << 0;
...
void GCS_MAVLINK::send_heartbeat_as_data16(uint8_t type, uint8_t base_mode, uint32_t custom_mode, uint8_t system_status, 
                                  int32_t lat, int32_t lon, int16_t alt, uint16_t heading, uint32_t gps_time) 
{

    uint8_t payload[16] = {0};

    // byte #0: status + armed\disarmed + airborne
    uint8_t b0 = system_status;
    ...
    // byte #1: control_mode 
    payload[1] = static_cast<uint8_t>(custom_mode);
    ...
    // bytes #10-11: alt in decimeters
    payload[10] = (alt >> 8) & 0xFF;
    payload[11] = alt & 0xFF;

    // bytes #12-13: yaw in decidegrees
    payload[12] = (heading >> 8) & 0xFF;
    payload[13] = heading & 0xFF;


    mavlink_msg_data16_send(
        chan,
        91,              // Telemetry packet id
        16,              // payload length
        payload);    
